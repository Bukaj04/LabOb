...
class TenAsteroid : public Asteroid {
public:
	TenAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 225; }
	void Draw() const override {
		Renderer::Instance().DrawPoly(transform.position, 10, GetRadius(), transform.rotation);
	}
};

// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, DECAGON = 10, RANDOM = 0 };
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
	...
	case AsteroidShape::DECAGON:
		return std::make_unique<TenAsteroid>(w, h);
	default: {
...
enum class WeaponType { LASER, DOUBLE, BULLET, COUNT };
class Projectile {
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			static constexpr float GAP = 10.f; // Odstęp między prostokątami

			// Pierwszy prostokąt (po lewej)
			Rectangle lr1 = {
				transform.position.x - GAP - 2.f,  // x: pozycja środka minus odstęp
				transform.position.y - LASER_LENGTH,
				4.f,
				LASER_LENGTH
			};

			// Drugi prostokąt (po prawej)
			Rectangle lr2 = {
				transform.position.x + GAP - 2.f,  // x: pozycja środka plus odstęp
				transform.position.y - LASER_LENGTH,
				4.f,
				LASER_LENGTH
			};

			DrawRectangleRec(lr1, GREEN);  // Np. niebieski
			DrawRectangleRec(lr2, GREEN);
		}
	}
	Vector2 GetPosition() const {
		return transform.position;
	...
	float speed)
{
	...
	else if (wt == WeaponType::DOUBLE) {
		return Projectile(pos, vel, 15, wt);
	}
	else {
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			if (IsKeyPressed(KEY_NINE)) {
				currentShape = AsteroidShape::DECAGON;
			}
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						if (currentWeapon == WeaponType::DOUBLE) {
							// Create two projectiles side by side
							projectiles.push_back(MakeProjectile(currentWeapon,
								Vector2{ p.x - 15.f, p.y }, projSpeed));
							projectiles.push_back(MakeProjectile(currentWeapon,
								Vector2{ p.x + 15.f, p.y }, projSpeed));
						}
						else {
			...
			// Render everything
			{
				...
				case WeaponType::DOUBLE: weaponName = "DOUBLE LASER"; break;
